...
# ✅ Relative imports (work when running from inside crew_mcp_project/)
...
# 🧩 Optional: Import the StorageProtect client if it exists
try:
    from ..ba_client_storage.sp_rest_client import StorageProtectClient
except ImportError:
    StorageProtectClient = None
...
# IBM Storage Protect REST config (adjust if needed)
...
def perform_backup(pg: str, mode: str = "cli"):
    ...
    if mode == "cli":
        ...
        if StorageProtectClient is None:
            raise HTTPException(status_code=500, detail="StorageProtectClient module not found")
